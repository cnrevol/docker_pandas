...
class AllocateActionx:
    ...
    def update_talbe(self, table_name, table_df):
        ...
        if not self.db.upsert_table(table_df,table_name):
            ...
            raise CustomException(f'Save allocate result {self.region} table {table_name} failed .')
    ...
    def cod_match(self, src_df, src_name, dest_dfs_dict, rules):
        ...
        """
        for _, row in rules.iterrows():
            self.logger.debug(f"cod_aloc_cn ")
    ...
    def search_data(self, src_df, src_name, dest_dfs_dict, row, payment):
        ...
        for ind_s, row_s in src_df_ts.iterrows():
            ...
            amount_index_dic = {}
            combination_sums = {}
            if row_s["aloc_comments"] is None or row_s["aloc_comments"] == "":
                # self.logger.debug(f'srcloop before extract value df index {ind_s} loop count :{s_loop_cnt}')
                src_values = self.extract_rule_values(
                    d_row=row_s,
                    fields=rule_item.src_fields,
                    rule_list=rule_item.src_rule_list,
                ...
                is_partial = False
                # filter customer_id
                src_df_td_f_cid = src_df_td[src_df_td["customer_id"]==row_s["customer_id"]]
                for ind_d, row_d in src_df_td_f_cid.iterrows():
                # for ind_d, row_d in src_df_td.iterrows():
                    if row_d["aloc_comments"] is None or row_d["aloc_comments"] == "":
                        # 在部分partial匹配的情况下，需要完全查找完才行，所以不执行break。
                        is_partial = self.has_partial(
                            rule_list=rule_item.dest_rule_list
                        )
                        dest_values = self.extract_rule_values(
                            d_row=row_d,
                        ...
                        # 判断配置字段值是否匹配
                        bool_m, combi_ids = self.compare_row_data(
                            src_values=src_values,
                            dest_values=dest_values,
                            rule_list=rule_item.src_rule_list,
                            amount_ind_dic=amount_index_dic,
                        ...
                        if bool_m:
                            self.set_result_fields(
                                rule_item.result_comment,
                                src_df_ts,
                                src_df_td,
                                ind_s,
                                src_values,
                            ...
                            if not is_partial:
                                break
                # 如果cid相同，so也相同的字段，全部被识别为soPartial匹配
                if is_partial:
                    if amount_index_dic:
                        pcombi_ids = [(ind_d, 0) for ind_d in amount_index_dic]
                        self.set_partial_result_fields(
                            rule_item.result_comment,
                            src_df_ts,
                            src_df_td,
                            ind_s,
    ...
    def search_data2(self, src_df, src_name, dest_dfs_dict, row, payment):
        """ "
        执行查找逻辑
        """
        # 取得配置信息
        ...
        # 4.cid匹配  amount 合计相反
        rule_item.src_rule_list
        rule_item.dest_rule_list
        rule_item.src_fields
        rule_item.dest_fields
        
        # cid so amount
        if set(rule_item.src_fields)==set(self.L_CID+self.L_INO+self.L_AMT) and set(rule_item.dest_fields)==set(self.L_CID+self.L_INO+self.L_AMT):
            customer_id=rule_item.src_fields[0]
            
            for index, rule_dict in enumerate(rule_item.src_rule_list):
            ...
            pass
        # cid so amount
        elif set(rule_item.src_fields)==set(self.L_CID+self.L_SO+self.L_AMT) and set(rule_item.dest_fields)==set(self.L_CID+self.L_INO+self.L_AMT):

            pass
        # cid so amount
        elif set(rule_item.src_fields)==set(self.L_CID+self.L_INO+self.L_AMT) and set(rule_item.dest_fields)==set(self.L_CID+self.L_SO+self.L_AMT):
            pass

        # cid so
        elif set(rule_item.src_fields)==set(self.L_CID+self.L_INO) and set(rule_item.dest_fields)==set(self.L_CID+self.L_SO):

            pass
        # cid so
        elif set(rule_item.src_fields)==set(self.L_CID+self.L_SO) and set(rule_item.dest_fields)==set(self.L_CID+self.L_INO):

            pass
        # cid so
        elif set(rule_item.src_fields)==set(self.L_CID+self.L_SO) and set(rule_item.dest_fields)==set(self.L_CID+self.L_SO):

            pass
        # cid amount
        elif set(rule_item.src_fields)==set(self.L_CID+self.L_AMT) and set(rule_item.dest_fields)==set(self.L_CID+self.L_AMT):

            pass
        # cid
        elif set(rule_item.src_fields)==set(self.L_CID) and set(rule_item.dest_fields)==set(self.L_CID):
        ...
        for index, rule_dict in enumerate(rule_item.dest_rule_list):
    ...
    def has_partial(self, rule_list):
        """
        判断是否为 连续部分匹配的定义
        """
        for index, rule_dict in enumerate(rule_list):
            match = rule_dict[self.R_MATCH]
            if match == self.RM_PAT:
                return True

    def set_partial_result_fields(
        self, result_comment, src_df_ts, src_df_td, ind_s, src_values, row_s, combi_ids
    ):
        """
        cid相同，so也相同的记录
        设定结果字段
        ...
        so = src_values[-1]
        self.set_df_row_result_fields(result_comment, src_df_ts, ind_s, 0, g_id, so)
        cid = row_s["customer_id"]
        for s_id, amt_dif in combi_ids:
            self.set_df_row_result_fields(
                result_comment, src_df_td, s_id, amt_dif, g_id, ""
            )

        self.logger.debug(
            f"Allocate partial data matched.  customer id:{cid} cmt:{result_comment}"
        )
    ...
    def set_result_fields(
        ...
        """
        g_id = self.generate_group()
        # 规定 配置项的最后一个值是sales order
        # ,df_s,df_d,ind_s,ind_d,match_list
        so = src_values[-1]
        self.set_df_row_result_fields(result_comment, src_df_ts, ind_s, 0, g_id, so)
        cid = row_d["customer_id"]
        # 多条记录匹配
        if combi_ids:
            for s_id, amt_dif in combi_ids:
            ...
            self.logger.debug(
                f"Allocate data matched.  customer id:{cid} cmt:{result_comment}"
            )
        else:
            self.set_df_row_result_fields(result_comment, src_df_td, ind_d, 0, g_id, so)
            self.logger.debug(
                f"Allocate data matched. salesorder:{so} customer id:{cid} cmt:{result_comment}"
            )

    def set_df_row_result_fields(self, result_comment, r_df, ind, amt_dif, g_id, so):
        """
        设定结果字段
        """
        r_df.loc[ind, "so_standard"] = so
        r_df.loc[ind, "aloc_comments"] = result_comment
        r_df.loc[ind, "aloc_status"] = self.set_aloc_status(result_comment)
        r_df.loc[ind, "amount_diff"] = amt_dif
        r_df.loc[ind, "aloc_group"] = g_id
    ...
    def set_aloc_status(self, r_comment):
        ...
        以外时 判定为so match
        """
        # if self.CMT_PAR in r_comment or self.CMT_SEQ in r_comment:
        if any(keyword in r_comment for keyword in [self.CMT_PAR, self.CMT_SEQ]):
    ...
    def find_equal_combinations_in(self, src_df, dst_df):
        """
        combinations(range(len(amounts1)), r) 会生成从 amounts1 列表中取 r 个元素的所有可能组合的索引。
        对于每个索引组合 combo,我们使用列表解析 [indices1[i] for i in combo] 从 indices1 列表中获取对应的索引。
        同时,我们使用 sum([amounts1[i] for i in combo]) 计算该组合的和。
        最后,我们将计算出的组合和和对应的索引作为一个元组添加到 combos1 或 combos2 中。
        """
        equal_combinations = []
            ...
            if len(amounts1) < data_edge:
                ...
                for cid2, amounts2, indices2 in dst_df[
                    ...
                    if len(amounts2) < data_edge:
                        if cid1 == cid2:
                            # self.logger.debug(f'combinations start customer id: {cid1} .amt1 cnts: {len(amounts1)} amt2 cnts: {len(amounts2)}')
                            combos1 = []
                            # 计算所有id组合的金额合计，保存
                            for r in range(1, len(amounts1) + 1):
                                if r <= sum_range or sum_range > len(amounts1) - r:
                                    combos1.extend(
                                        [
                                            (
                            ...
                            for r in range(1, len(amounts2) + 1):
                                if r <= sum_range or sum_range > len(amounts2) - r:
                                    combos2.extend(
                                        [
                                            (
                            ...
                            for combo1, indices1 in combos1:
                                for combo2, indices2 in combos2:
                                    if abs(
                                        Decimal(str(combo1)) + Decimal(str(combo2))
                                    ) <= Decimal(str(self.tole)):
                                        ...
                                        # self.logger.debug(f'combinations matched customer id: {cid1} . amount sum : {combo1} , {combo2}')
                                        break
    ...
    #     src_fields = str(row['src_fields']).split(self.ITEM_SPLIT)

    #     src_data_type = row['src_data_type']
    #     dst_table = row['dest_data']
    #     dst_fields = str(row['dest_fields']).split(self.ITEM_SPLIT)
    #     src_match_rules = row['src_match_rules']
    #     dst_match_rules = row['dst_match_rules']
    #     dest_data_type = row['dest_data_type']

    #     result_field = row['result_field']
    #     result_comment  = row['result_comment_field']
    #     src_rule_list = self.parse_rule_string(src_match_rules)
    #     dst_rule_list = self.parse_rule_string(dst_match_rules)
    #     return rule_name,rule_seq,src_fields,src_data_type,dst_table,dst_fields,dest_data_type,result_field,result_comment,src_rule_list,dst_rule_list
    ...
    #         'result_comment': row['result_comment_field'],
    #         'src_rule_list': self.parse_rule_string(row['src_match_rules']),
    #         'dst_rule_list': self.parse_rule_string(row['dst_match_rules'])
    #     }
    ...
    def compare_row_data(
        self,
        src_values,
        dest_values,
        rule_list,
        amount_ind_dic=None,
        combination_sums=None,
        ind_s=0,
        ind_d=0,
        row_s=None,
        row_d=None,
        ...
        combi_ids = []
        for index, rule_dict in enumerate(rule_list):
            src_value = src_values[index]
            dest_value = dest_values[index]
            match = rule_dict[self.R_MATCH]
            if match == self.RM_EQ:
                if self.is_equal(src_value=str(src_value), dest_value=str(dest_value)):
                    ret_list.append(True)
                else:
                    return False, combi_ids
            elif match == self.RM_PAT:
                if all(ret_list):
                    if self.is_equal(
                        src_value=str(src_value), dest_value=str(dest_value)
                    ):
                        self.add_so_list(
            ...
            elif match == self.RM_OPP:
                if abs(Decimal(str(src_value)) + Decimal(str(dest_value))) <= Decimal(
                    str(self.tole)
                ):
                    ...
                    return False, combi_ids
            # 单纯合计匹配，ADV时
            elif match == self.RM_SUM:
                # 如果其他值匹配成功，且存在金额合计时，
                if all(ret_list):
                    # if self.is_adv_row(row_d=row_d):
                    ...
                    combi_ids = self.hold_combination_sums(
                        amount_ind_dic=amount_ind_dic,
                        sum_amount=src_value,
                        combination_sums=combination_sums,
                    )
                    if combi_ids:
                        ret_list.append(True)
                    ...
                    return False, combi_ids
            # 顺销的处理，Term时
            # TODO 发票时间，账期，入账时间的条件是否需要限制
            elif match == self.RM_SEQ:
                if all(ret_list):
                    # 入账数据和发票数据相邻的情况下，计算顺销
                    amount_ind_dic = self.add_amount_list(
                        amount_ind_dic=amount_ind_dic,
                        ind_d=ind_d,
                        row_d=row_d,
                    ...
                    # 检查 ind_d list 是否与 ind_s 相邻
                    combi_ids = self.hold_sequential_sums(
                        amount_ind_dic=amount_ind_dic, sum_amount=src_value, ind_s=ind_s
                    )
    ...
    def hold_combination_sums(self, amount_ind_dic, sum_amount, combination_sums):
        ...
        for combination_length in range(1, len(ids) + 1):
            if (
                ...
                for combination_ids in combinations(ids, combination_length):
    ...
    def hold_sequential_sums(self, amount_ind_dic, sum_amount, ind_s):
        """
        累加顺序发票数据金额
        TODO 不连续的情况下需要中断，重算
        """
        ...
        if amount_dif <= Decimal(str(self.tole)):
    ...
    def filter_payment(self, src_df, payment):
        ...
        if payment == self.PMT_ADV:
            payment = self.comm_util.get_def_data_by_name(self.def_data, "payment_adv").split(",")
            mask = src_df["payment"].isin(payment)
            obj_df = src_df[mask]
            opst_df = src_df[~mask]
        elif payment == self.PMT_TERM:
            payment = self.comm_util.get_def_data_by_name(self.def_data, "payment_terms")
    ...
    def get_src_bl_table(self, table_name, region):
        ...
        if table_name.lower() == "bl_aging_history":
            ...
            # AND (ipf_status !='data_posted' OR ipf_status is null)"""
        parameters = (region,)
        rst = self.db.execute_query_to_pandas(sql_query, parameters)
        return rst
    ...
    def init_src_table_cn_query(self):
        ...
        sql_query = """SELECT
                        ...
                        order by cn_index
                        """
        # AND ag.doc_type IN ('SLI','ACB','ARR','TCR','AHB','AHR','AAB','STF','A60','SBD','SLC')
        return sql_query
